<!--
	Twenty by HTML5 UP
	html5up.net | @ajlkn
	Free for personal and commercial use under the CCA 3.0 license (html5up.net/license)
-->

<!--
	CS4753 - E-commerce
	Semester Project
	Group members:  Andrew Roberts (ajr5re), Ryan Donovan (rad9vy)

	Current Version:  Milestone 1
-->

<?php
	include('sessioncheck.php');
?>

<!DOCTYPE HTML>

<html>
	<head>
		<title>Charlottesville Bands</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<!--[if lte IE 8]><script src="assets/js/ie/html5shiv.js"></script><![endif]-->
		<link rel="stylesheet" href="assets/css/main.css" />
		<!--[if lte IE 8]><link rel="stylesheet" href="assets/css/ie8.css" /><![endif]-->
		<!--[if lte IE 9]><link rel="stylesheet" href="assets/css/ie9.css" /><![endif]-->
	</head>
	<body class="index">
		<div id="page-wrapper">

			<!-- Header -->
				<header id="header" class="alt">
					<h1 id="logo"><a href="index.php"><span></span></a></h1>
					<nav id="nav">
						<ul>
							<li class="gray current"><a href="index.php">Home</a></li>
							<li class="gray"><a href="aboutpage.php">About Us</a></li>
							<li class="gray"><a href="loginpage.php" class="button special">Login</a></li>
							<li class="gray"><a href="signuppage.php" class="button special">Sign Up</a></li>
						</ul>
					</nav>
				</header>

			<!-- Banner -->
				<section id="banner">

					<!--
						".inner" is set up as an inline-block so it automatically expands
						in both directions to fit whatever's inside it. This means it won't
						automatically wrap lines, so be sure to use line breaks where
						appropriate (<br />).
					-->
					<div class="inner">

						<header>
							<h2>Charlottesville Bands</h2>
						</header>
						<p>Live entertainment for Charlottesville,
						<br />
						by Charlottesville.</p>
						<footer>
							<ul class="buttons vertical">
								<li><a href="#main" class="button fit scrolly">Tell Me More</a></li>
							</ul>
						</footer>

					</div>

				</section>

			<!-- Main -->
				<article id="main">

					<header class="special container">
						<span class="icon fa-music"></span>
						<h2>Book, interact, and connect with the best local bands Charlottesville has to offer.</h2>
						<p>
							As Charlottesville's predominant online booking agent, more customers and bands now trust</br>
							Charlottesville Bands. We connect you to local bands that fit perfectly for your next event,</br>
							tailored to your specifications. Book your dream band directly through our online interface.</p>
					</header>

					<!-- Three -->
						<section class="wrapper style3 container special">

							<header class="major">
								<h2>Bands <strong>ready to perform.</strong></h2>
							</header>

							<div class="row">
								<div class="6u 12u(narrower)">

									<section>
										<a href="signuppage.php" class="image featured"><img src="images/crd_img.jpg" width = "250" height = "300" alt="" /></a>
										<header>
											<h3>Corduroy</h3>
										</header>
										<p>Rock band formed by UVA students</p>
									</section>

								</div>
								<div class="6u 12u(narrower)">

									<section>
										<a href="signuppage.php" class="image featured"><img src="images/kendall_street_img.jpg" width = "250" height = "300" alt="" /></a>
										<header>
											<h3>Kendall Street Company</h3>
										</header>
										<p>UVA's favorite Jam Band</p>
									</section>

								</div>
							</div>
							<div class="row">
								<div class="6u 12u(narrower)">

									<section>
										<a href="signuppage.php" class="image featured"><img src="images/ajr_img.jpg" width = "250" height = "300" alt="" /></a>
										<header>
											<h3>The Andrew Roberts Experience</h3>
										</header>
										<p>Charlottesville's Predominant Jazz Sextet</p>
									</section>

								</div>
								<div class="6u 12u(narrower)">

									<section>
										<a href="signuppage.php" class="image featured"><img src="images/dmb_img.jpeg" width = "250" height = "300" alt="" /></a>
										<header>
											<h3>Dave Matthews Band</h3>
										</header>
										<p>The pride of Charlottesville</p>
									</section>

								</div>
							</div>

							<footer class="major">
								<ul class="buttons">
									<li><a href="signuppage.php" class="button">See More Bands</a></li>
								</ul>
							</footer>

						</section>

				</article>

			<!-- CTA -->
				<section id="cta">

					<header>
						<h2>Ready to <strong>book</strong>?</h2>
						<p>Sign up or login to get started.</p>
					</header>
					<footer>
						<ul class="buttons">
							<li><a href="signuppage.php" class="button special">Sign up</a></li>
							<li><a href="loginpage.php" class="button">Login</a></li>
						</ul>
					</footer>

				</section>

			<!-- Footer -->
				<footer id="footer">

					<ul class="icons">
					</ul>

					<ul class="copyright">
						<li>&copy; Charlottesville Bands</li><li>Design: <a href="http://html5up.net">HTML5 UP</a></li>
					</ul>

				</footer>

		</div>

		<!-- Scripts -->
			<script src="assets/js/jquery.min.js"></script>
			<script src="assets/js/jquery.dropotron.min.js"></script>
			<script src="assets/js/jquery.scrolly.min.js"></script>
			<script src="assets/js/jquery.scrollgress.min.js"></script>
			<script src="assets/js/skel.min.js"></script>
			<script src="assets/js/util.js"></script>
			<!--[if lte IE 8]><script src="assets/js/ie/respond.min.js"></script><![endif]-->
			<script src="assets/js/main.js"></script>
	</body>
</html>
